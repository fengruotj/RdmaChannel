 ...
 */
public class RdmaPassiveReadClient implements RdmaEndpointFactory<RdmaPassiveReadClient.CustomClientEndpoint> {
	...
	public RdmaPassiveReadClient(String host, int port, int size, int loop) throws IOException{
		this.endpointGroup = new RdmaPassiveEndpointGroup<RdmaPassiveReadClient.CustomClientEndpoint>(1, 10, 4, 40);
	...
	public void run() throws Exception {
		...
		RdmaPassiveReadClient.CustomClientEndpoint endpoint = endpointGroup.createEndpoint();
		...
		for (int i = 10; i <= 100; ){
			...
			dataBuf.clear();
			System.out.println("RdmaPassiveReadClient::read memory from server: " + dataBuf.asCharBuffer().toString());
			i += 10;
		}
	...
	public static void main(String[] args) throws Exception {
		...
		RdmaPassiveReadClient client = new RdmaPassiveReadClient(cmdLine.getIp(), cmdLine.getPort(), cmdLine.getSize(), cmdLine.getLoop());
		client.run();
	}
